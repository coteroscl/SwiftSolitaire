...
// MARK: Double Tap
private let SPACING = CGFloat(UIScreen.main.bounds.width > 750 ? 10.0 : 3.0)
let CARD_WIDTH = CGFloat((UIScreen.main.bounds.width - CGFloat(7.0 * SPACING)) / 7.0)
let CARD_HEIGHT = CARD_WIDTH * 1.42
extension Selector {
    fileprivate static let handleTap = #selector(SolitaireGameView.newDealAction)
}
final class SolitaireGameView: UIView {
    ...
    private func initStackViews() {
        let baseRect = CGRect(
            x: 4.0, y: scaled(value: 110.0), width: CARD_WIDTH, height: CARD_HEIGHT)
        var foundationRect = baseRect
        for index in 0..<4 {
            let stackView = FoundationCardStackView(
                frame: foundationRect, cards: Model.sharedInstance.foundationStacks[index])
        ...
        foundationRect = foundationRect.offsetBy(dx: CGFloat(CARD_WIDTH + SPACING), dy: 0.0)
        self.talonStackView = TalonCardStackView(
            frame: foundationRect, cards: Model.sharedInstance.talonStack)
        self.addSubview(self.talonStackView)

        foundationRect = foundationRect.offsetBy(dx: CGFloat(CARD_WIDTH + SPACING), dy: 0.0)
        self.stockStackView = StockCardStackView(
            frame: foundationRect, cards: Model.sharedInstance.stockStack)
        self.addSubview(self.stockStackView)

        var gameStackRect = baseRect.offsetBy(
            dx: 0.0, dy: CGFloat(CARD_HEIGHT + scaled(value: 12.0)))
        self.baseTableauFrameRect = gameStackRect
        for index in 0..<7 {
            let stackView = TableauStackView(
                frame: gameStackRect, cards: Model.sharedInstance.tableauStacks[index])
            self.addSubview(stackView)
        ...
        let buttonFrame = CGRect(
            x: 1.0, y: scaled(value: 60.0), width: scaled(value: 70.0), height: scaled(value: 30.0))
    ...
    private func dealCards() {
        ...
        for outerIndex in 0..<7 {
            ...
            for innerIndex in (0...outerIndex) {
                Model.sharedInstance.tableauStacks[outerIndex].addCard(
                    card: Card(
                        value: Model.sharedInstance.deck[cardValuesIndex],
                        faceUp: outerIndex == innerIndex))
                cardValuesIndex += 1
            }
        ...
        for _ in cardValuesIndex..<52 {
            Model.sharedInstance.stockStack.addCard(
                card: Card(value: Model.sharedInstance.deck[cardValuesIndex], faceUp: false))
            cardValuesIndex += 1
        }
    }
...
extension SolitaireGameView {
            ...
            for cardView in touchedView.subviews.reversed() {
                if let cardView = cardView as? CardView {
                    ...
                    if cardView.isFaceUp && cardView.point(inside: t, with: event) {
                        ...
                        if let index = stackDraggedFrom!.cards.cards.firstIndex(where: {
                            $0.value == dragCard.value
                        }) {
                            ...
                            for card in Model.sharedInstance.dragStack.cards {
                                let index = stackDraggedFrom!.cards.cards.firstIndex {
                                    $0.value == card.value
                                }
    ...
    override func touchesMoved(
        ...
        if let touch = touches.first {
            ...
            moveDragView(
                offset: CGPoint(
                    x: (currentPosition.x) - (oldLocation.x),
                    y: (currentPosition.y) - (oldLocation.y)))
        }
    }
    ...
    override func touchesEnded(
        ...
        if doingDrag {
            ...
            // Try dropping on tableau stacks
            for view in tableauStackViews where view != stackDraggedFrom! {
                ...
                if viewFrame.intersects(dragFrame) {
                    // FIXED: Validate entire sequence, not just first card
                    if let tableauStack = view.cards as? TableauStack,
                        tableauStack.canAcceptSequence(dragView.cards.cards)
                    {

                        // Wrap move in performMove for undo support
            ...
            // Try dropping on foundation stacks (single card only)
