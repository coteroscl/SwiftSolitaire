...
class LayeredStackView: CardStackView {
    ...
    override fileprivate func addCard(card: Card) {
        ...
        if self.subviews.count > 0 {
            ...
            let offset =
                previousCard.isFaceUp ? (CARD_HEIGHT * cardLayerOffset) : (CARD_HEIGHT * 0.07)
    ...
    override func refresh() {
        ...
        self.frame = CGRect(
            x: self.frame.minX, y: self.frame.minY, width: CARD_WIDTH, height: CARD_HEIGHT)
...
final class FoundationCardStackView: PiledStackView {
    ...
    func commonInit() {
        ...
        label.font = UIFont(
            name: "TrebuchetMS", size: screenheight * (screenheight > 900 ? 0.12 : 0.07))
...
final class StockCardStackView: PiledStackView {
    ...
    @objc func handleTap() -> Bool {
        if self.cards.topCard() != nil {
            // Flip card from stock to talon with undo support
            Game.sharedInstance.performMove {
                Game.sharedInstance.moveTopCard(
                    from: Model.sharedInstance.stockStack, to: Model.sharedInstance.talonStack,
                    faceUp: true, makeNewTopCardFaceup: false)
            }
            return true
        } else {
            // Recycle talon back to stock with undo support
            Game.sharedInstance.performMove {
                Game.sharedInstance.copyCards(
                    from: Model.sharedInstance.talonStack, to: Model.sharedInstance.stockStack)
